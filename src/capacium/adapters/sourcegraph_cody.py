...
class SourcegraphCodyAdapter(FrameworkAdapter):

    def __init__(self):
        self.storage = StorageManager()
        self.config_path = Path.home() / ".cody" / "mcp.json"

    def install_skill(self, cap_name: str, version: str, source_dir: Path, owner: str = "global") -> bool:
        print("Sourcegraph Cody does not support skill symlinking. Use 'mcp-server' kind.")
        return False
