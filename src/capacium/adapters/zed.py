"""Zed Editor MCP adapter.

Config: ~/.config/zed/settings.json → context_servers section
"""
import shutil
...
class ZedAdapter(FrameworkAdapter):

    def __init__(self):
        self.storage = StorageManager()
        self.config_path = Path.home() / ".config" / "zed" / "settings.json"

    def install_skill(self, cap_name: str, version: str, source_dir: Path, owner: str = "global") -> bool:
        print("Zed Editor does not support skill symlinking. Use 'mcp-server' kind.")
        return False
    ...
    def install_mcp_server(self, cap_name: str, version: str, source_dir: Path, owner: str = "global") -> bool:
        ...
        return McpConfigPatcher.inject_json_mcp_server(
            ...
            server_key=cap_name,
            mcp_section_key="context_servers",
            cap_name=cap_name,
    ...
    def remove_mcp_server(self, cap_name: str, owner: str = "global") -> bool:
        return McpConfigPatcher.remove_json_mcp_server(
            self.config_path, cap_name, "context_servers",
        )
