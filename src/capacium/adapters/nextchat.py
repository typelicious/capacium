"""NextChat MCP adapter.

Config: ~/.nextchat/mcp_config.json
"""
import shutil
...
class NextChatAdapter(FrameworkAdapter):

    def __init__(self):
        self.storage = StorageManager()
        self.config_path = Path.home() / ".nextchat" / "mcp_config.json"

    def install_skill(self, cap_name: str, version: str, source_dir: Path, owner: str = "global") -> bool:
        print("NextChat does not support skill symlinking. Use 'mcp-server' kind.")
        return False
