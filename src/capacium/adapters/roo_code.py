"""Roo Code (formerly Roo Cline) MCP adapter.

Config: ~/.roo-code/mcp.json
Roo Code is a fork of Cline with enhanced agent orchestration.
"""
import shutil
...
class RooCodeAdapter(FrameworkAdapter):
    ...
    def __init__(self):
        ...
        self.config_path = Path.home() / ".roo-code" / "mcp.json"

    def install_skill(self, cap_name: str, version: str, source_dir: Path, owner: str = "global") -> bool:
        print("Roo Code does not support skill symlinking. Use 'mcp-server' kind.")
        return False
