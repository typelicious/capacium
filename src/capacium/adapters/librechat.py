...
Config: ~/librechat/librechat.yaml → mcpServers section
"""
import shutil
...
class LibreChatAdapter(FrameworkAdapter):

    def __init__(self):
        self.storage = StorageManager()
        # LibreChat uses YAML, but we store MCP config in a sidecar JSON
        # that the Docker compose injects
        self.config_path = Path.home() / ".librechat" / "mcp_servers.json"

    def install_skill(self, cap_name: str, version: str, source_dir: Path, owner: str = "global") -> bool:
        print("LibreChat does not support skill symlinking. Use 'mcp-server' kind.")
        return False
